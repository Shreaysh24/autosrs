...
function Timeline({ currentStep, setCurrentStep, step }) {
  ...
  return (
    <div className=" bg-transparent w-full py-5 text-xs md:text-base rounded-2xl flex justify-start items-center">
      ...
      <div
        className={`w-4 md:w-5 border-t-2    ${
          currentStep >= 12 ? "border-[#0253b9]" : "border-[#c5c5c59c]"
        }`}
      ></div>
            <div className="flex gap-5 ">
        <div
        onClick={() => handleClick(12)}
          className={`rounded-full ${step<12?"pointer-events-none opacity-40":""} cursor-pointer p-2 md:p-3 w-6 h-6 md:w-8 md:h-8 flex justify-center items-center font-semibold ${
            currentStep >= 12
              ? "text-white bg-[#0253b9]"
              : "text-gray-300 bg-[#00000029] custom-border3"
          }`}
